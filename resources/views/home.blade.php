            ...
            <div class="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-12">
                <div>
                    <div class="flex items-center gap-3 mb-3">
                        ...
                        <span class="font-cursive text-xl text-green-700">Momen Berharga</span>
                    </div>
                    <h2 class="text-3xl md:text-4xl font-bold text-gray-900 leading-tight">Galeri Foto</h2>
                    <p class="text-gray-500 mt-2">Dokumentasi kegiatan dan keindahan Nagari Pematang Panjang</p>
                </div>
                <a href="{{ route('gallery.index') }}"
                    class="inline-flex items-center gap-2 text-green-700 hover:text-green-900 font-semibold border border-green-200 hover:border-green-400 px-5 py-2.5 rounded-xl transition-all duration-300 hover:bg-green-50 whitespace-nowrap flex-shrink-0">
                    Lihat Semua
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/>
                    </svg>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                @foreach ($galleries as $gallery)
                    <a href="{{ route('gallery.show', $gallery) }}"
                        class="group relative overflow-hidden rounded-2xl shadow-sm hover:shadow-xl transition-all duration-500 block aspect-[4/3]">
                        <img src="{{ Storage::url($gallery->gambar) }}" alt="{{ $gallery->judul }}"
                            class="absolute inset-0 w-full h-full object-cover group-hover:scale-110 transition-transform duration-700">
                        {{-- Gradient overlay --}}
                        <div class="absolute inset-0 bg-gradient-to-t from-black/70 via-black/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-400"></div>
                        {{-- Caption slide up --}}
                        <div class="absolute bottom-0 left-0 right-0 p-5 transform translate-y-full group-hover:translate-y-0 transition-transform duration-400">
                            <h3 class="text-white font-semibold text-sm leading-tight">{{ $gallery->judul }}</h3>
                            @if ($gallery->deskripsi)
                                <p class="text-white/70 text-xs mt-1">{{ Str::limit($gallery->deskripsi, 60) }}</p>
                            @endif
                        </div>
                    </a>
                @endforeach
        ...
        <div class="max-w-7xl mx-auto px-6 sm:px-10 lg:px-16">
            ...
            <div class="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-12">
                <div>
                    <div class="flex items-center gap-3 mb-3">
                        <div class="w-8 h-px bg-green-600"></div>
                        <span class="font-cursive text-xl text-green-700">Produk Lokal Berkualitas</span>
                    </div>
                    <h2 class="text-3xl md:text-4xl font-bold text-gray-900 leading-tight">UMKM Unggulan</h2>
                    <p class="text-gray-500 mt-2">Usaha Mikro Kecil Menengah yang menjadi kebanggaan nagari</p>
                </div>
                <a href="{{ route('umkm.index') }}"
                    class="inline-flex items-center gap-2 text-green-700 hover:text-green-900 font-semibold border border-green-200 hover:border-green-400 px-5 py-2.5 rounded-xl transition-all duration-300 hover:bg-green-50 whitespace-nowrap flex-shrink-0">
                    Lihat Semua UMKM
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                @foreach ($umkms as $umkm)
                    <div class="group bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-lg hover:-translate-y-1 transition-all duration-300 overflow-hidden flex flex-col">
                        {{-- Gambar --}}
                        @if ($umkm->gambar)
                        ...
                        @else
                            <div class="h-2 bg-gradient-to-r from-green-600 to-emerald-400 flex-shrink-0"></div>
                        @endif

                        {{-- Konten --}}
                        <div class="p-5 flex flex-col flex-1">
                            <h3 class="text-base font-bold text-gray-900 mb-2 group-hover:text-green-700 transition-colors leading-snug">
                                {{ $umkm->nama }}
                            </h3>
                            <p class="text-gray-500 text-sm line-clamp-2 mb-4 flex-1">{{ Str::limit($umkm->deskripsi, 100) }}</p>
                                ...
                                <div class="flex gap-2">
                                    @if ($umkm->whatsapp)
                                        <a href="https://wa.me/{{ $umkm->whatsapp }}" target="_blank"
                                            class="w-7 h-7 bg-green-50 hover:bg-green-100 rounded-lg flex items-center justify-center text-green-600 transition-colors">
                                    ...
                                    @if ($umkm->instagram)
                                        <a href="{{ $umkm->instagram }}" target="_blank"
                                            class="w-7 h-7 bg-pink-50 hover:bg-pink-100 rounded-lg flex items-center justify-center text-pink-500 transition-colors">
        ...
        <div class="max-w-7xl mx-auto px-6 sm:px-10 lg:px-16">
            ...
            <div class="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-12">
                <div>
                    <div class="flex items-center gap-3 mb-3">
                        <div class="w-8 h-px bg-green-600"></div>
                        <span class="font-cursive text-xl text-green-700">Informasi Terkini</span>
                    </div>
                    <h2 class="text-3xl md:text-4xl font-bold text-gray-900 leading-tight">Artikel & Berita Terbaru</h2>
                    <p class="text-gray-500 mt-2">Informasi terkini seputar kegiatan dan perkembangan nagari</p>
                </div>
    ...
    <section class="relative overflow-hidden py-20" style="background-color: #f0f4f0;">
        ...
        <div class="relative max-w-7xl mx-auto px-6 sm:px-10 lg:px-16">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-center">
                ...
                {{-- ===== Kolom Kanan: Gambar blend bersih ===== --}}
                <div class="lg:col-span-7">

                    {{-- Foto blend tanpa teks overlay --}}
                    <div class="relative rounded-3xl overflow-hidden" style="min-height: 480px;">

                        {{-- Foto --}}
                        <img src="{{ asset('images/images/kadunoicon.png') }}"
                            alt="Pengaduan Masyarakat Nagari Pematang Panjang"
                            class="absolute inset-0 w-full h-full object-cover">

                        {{-- Soft fade ke kiri & ke bawah agar blend dengan background section --}}
                        <div class="absolute inset-0" style="background: linear-gradient(to left, transparent 40%, #f0f4f0 100%), linear-gradient(to top, #f0f4f0 0%, transparent 25%);"></div>
